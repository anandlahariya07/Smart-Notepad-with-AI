...
class Notepad:
    def __init__(self):
        ...
        l.append(00)
        l.append(00)
        l.append(cur_time2.second)

        t1 = Thread(target=self.changed, args=l)
        t1.start()
    ...
    def set_controllers(self):
        self.file_controller=FileController.File_Controller()
        self.db_controller=  DBController.DB_Conroller()
    ...
    def exit_func(self, event=None):
        try:
            ...
            messagebox.showerror("Application Error!","Unable to save the file")
            print(traceback.format_exc())
        finally:
            messagebox.showinfo("Thank You","You are Quitting")
    ...
    def save_file(self, event=None):
        try:
            ...
            messagebox.showerror("File Error!","Cannot save file, Try Again!")
            print(traceback.format_exc())
    ...
    def changed(self, h2,m2,c, event=None):
        while True:
            ...
            if self.text_editor.edit_modified():
                ...
                self.status_bar.config(text=f'Characters: {character}  Words: {words}  Time: {str(h1) + ":" + str(m1) + ":" + str(s1)[:2]}  Running Time: {str(h2) + ":" + str(m2-1) + ":" + str(s2)[:2]}')
            else:
                words = len(self.text_editor.get(1.0, 'end-1c').split())
                character = len(self.text_editor.get(1.0, 'end-1c'))
                self.status_bar.config(text=f'Characters: {character}  Words: {words}  Time: {str(h1) + ":" + str(m1) + ":" + str(s1)[:2]}  Running Time: {str(h2) + ":" + str(m2-1) + ":" + str(s2)[:2]}')
    ...
    def find_func(self, event=None):
        ...
        self.replace_button=ttk.Button(self.find_frame,text='Replace',command=self.find)
    ...
    def say_something(self):
        try:
            ...
            if self.takeAudio=="":
                ...
                self.exit_func()
            elif self.takeAudio.lower()=='hey mojo bold text':
                self.change_bold()
            elif self.takeAudio.lower()=='hey mojo italic text':
                self.change_italic()
            elif self.takeAudio.lower()=='hey mojo open search':
                self.find_func()
                ...
                messagebox.showerror("Error","Sorry! audio doesn't matched")
            ...
            messagebox.showerror("Unknown Value Error","Speech Unrecognizable")
            print(traceback.format_exc())
        except WaitTimeoutError as wte:
            messagebox.showerror("Wait Timeout Error","Time Limit Exceeded")
            print(traceback.format_exc())
    ...
    def change_bold(self):
        self.text_property=tk.font.Font(font=self.text_editor)
    ...
    def secure_file(self, event=None):
        if self.db_status:
            ...
            self.text_label_pwd_label=  ttk.Label(self.file_frame,text='File Password: ')
            ...
            #button
            self.file_add_button= ttk.Button(self.file_frame,text='Add Files',command=self.add_file)
            self.file_open_button= ttk.Button(self.file_frame,text='Open Files', command=self.open_secure_file)
            self.file_remove_button= ttk.Button(self.file_frame,text='Remove Files',command=self.remove_secure_file)

            #label grid
            self.text_file_owner_label.grid(row=0,column=0,padx=4,pady=4)
            self.text_label_pwd_label.grid(row=1,column=0,padx=4,pady=4)
            ...
            self.fileList.configure(font='Loto')
            ...
            self.fileList.configure(selectbackground='black')
            ...
            messagebox.showerror("Database Error","Please connect with Database")
       ...
       try:
           ...
           if len(self.sel_file_index_tuple)==0:
               ...
               if pwd==self.db_controller.get_file_pwd(basename):
                   ...
                   messagebox.showerror("Invalid Password!", "You Entered wrong password")
       except:
    ...
    def remove_secure_file(self):
        try:
            ...
            if len(self.sel_file_index_tuple) != 0:
                ...
                messagebox.showinfo("Successful","File removed Successfully")
            else:
    ...
    def add_file(self):
        if self.file_owner_input.get()!="" and self.file_pwd_input.get!="":
            ...
            messagebox.showerror("Error!","Enter File owner and Password")
